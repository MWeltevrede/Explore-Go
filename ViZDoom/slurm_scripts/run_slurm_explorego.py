...
def run_slurm(run_description, args):
    ...
    for sbatch_file in sbatch_files:
        ...
        print(sbatch_fname)
        cmd = f'sbatch --job-name=vizdoom --time={TIME} --qos={QOS} --gres=gpu:a40:1 --ntasks=1 --cpus-per-task={CPUS_PER_TASK} --partition={PARTITION} --mem-per-cpu=6GB --account={ACCOUNT} --output={workdir}/{sbatch_fname}-slurm.out --error={workdir}/{sbatch_fname}-slurm.err slurm_vizdoom/{sbatch_fname}'
